...
public class PhysicEngineStepDefs implements En {
    ...
    public PhysicEngineStepDefs() {
        ...
        Given("the Configuration is loaded from {string}", (String cfgFile) -> config = new Configuration(cfgFile));
        ...
        Then("the PhysicEngine default world width is {double}",
                (Double width) -> Assert.assertEquals("The world width is not set to " + width, width, pe.getWorld().width, 0.0));
        Then("the PhysicEngine default world height is {double}",
                (Double height) -> Assert.assertEquals("The world height is not set to " + height, height, pe.getWorld().height, 0.0));
        And("the PhysicEngine default world gravity is {double},{double}", (Double vx, Double vy) -> {
            Vector2d vg = new Vector2d(vx, vy);
            Assert.assertEquals("The world gravity is not set to " + vg, vg, pe.getWorld().gravity);
        });
    }
