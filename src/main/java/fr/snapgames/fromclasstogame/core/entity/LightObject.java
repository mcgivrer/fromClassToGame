...
/**
 * <p>The `LightObject` will be used to simulate lights on the scene rendering.</p>
 * <p>To define a `LightObject`, you must set its name, its initial position, and its type. and then set some of its parameters</p>
 * <ul>
 *     <li><code>{@link LightObject#lightType}</code> the type of light (see {@link LightType})</li>
 *     <li><code>{@link LightObject#setForegroundColor(Color)}</code> the light color , a {@link Color} </li>
 *     <li><code>{@link LightObject#setIntensity(Double)}</code> the light intensity , a double value from 0.0 to 1.0) </li>
